...
We can make a correct proof by contradiction. Start by assuming the intended proposition is false and see if we can reach a contradiction, thereby proving the proposition true.

{\it Corrected proof.}
\begin{align}
  \frac{a+b}{2} &< \sqrt{ab} &\text{(original proposition negated)} \\
a+b &< 2\sqrt{ab} \\
a^2+2ab+b^2 &< 4ab\\
a^2-2ab+b^2 &< 0\\
  (a-b)^2 &< 0 &\text{(false since any squared value is non-negative)}
\end{align}

This proves the assumption $\frac{a+b}{2} < \sqrt{ab}$ is false, so its negation $\frac{a+b}{2} \geq \sqrt{ab}$ must be true.
